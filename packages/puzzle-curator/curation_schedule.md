# IPL 2026 — Curation Schedule v5

Generated by `gen_schedule_v5.py`.
- Yellow (1-Easy)  : Current squad / fan-base anchored
- Green  (2-Medium): Historical squad / legends / geographic
- Blue   (3-Hard)  : Stats / coaches / geographic deep-dive
- Purple (4-Expert): Cross-team / niche records / nomads

Status: `[ ]` pending · `[g]` generated · `[r]` reviewed · `[x]` published · `[!]` needs attention

## Items adjusted
| Yellow | Green | Blue | Purple | Ed |
| `team_players:MI:2026` | | | | 1 |
| `team_players:CSK:2026` | | | | 2 |
| `team_players:KKR:2026` | | | | 4 |


## Pre-IPL (24-27 Mar)

| Ed | Date | Day | Match | Yellow | Green | Blue | Purple | Status |
|---|---|---|---|---|---|---|---|---|
| 1 | 24 Mar | Tue | — | `team_players:MI:2026` | `country:Australia` | `top_run_scorers` | `played_both:CSK:RCB` | [x] |
| 2 | 25 Mar | Wed | — | `country:Sri Lanka` | `team_players:MI:2008` |`team_players:CSK:2025` | `purple_cap` | [x] |
| 3 | 26 Mar | Thu | — | `head_coaches:2008` | `team_owners` | `state:Kerala` | `multi_team:5` | [x] |
| 4 | 27 Mar | Fri | — | `legends:india` | `country:Afghanistan` | `team_players:KKR:2008`  | `most_ducks` | [ ] |

## Phase 1 — Match Days (28 Mar - 12 Apr)

| Ed | Date | Day | Match | Yellow | Green | Blue | Purple | Status |
|---|---|---|---|---|---|---|---|---|
| 5 | 28 Mar | Sat | RCB vs SRH | `team_players:SRH:2026` | `legends:overseas` | `batting_records` | `orange_cap` | [ ] |
| 6 | 29 Mar | Sun | MI vs KKR | `team_players:MI:2026` | `team_legends_bowling:CSK` | `country:Australia` | `multi_team:7`  | [ ] |
| 7 | 30 Mar | Mon | RR vs CSK | `team_players:RR:2026` | `costliest_player` | `state:Maharashtra` | `played_both:CSK:MI` | [ ] |
| 8 | 31 Mar | Tue | PBKS vs GT | `team_players:PBKS:2026` | `winning_captain` | `ranji:Mumbai` | `multi_team:5` | [ ] |
| 9 | 1 Apr | Wed | LSG vs DC | `team_players:LSG:2026` | `player_of_tournament` | `head_coaches` | `multi_team:7` | [ ] |
| 10 | 2 Apr | Thu | KKR vs SRH | `team_players:KKR:2026` | `country:South Africa` | `bowling_records` | `top_wicket_takers` | [ ] |
| 11 | 3 Apr | Fri | CSK vs PBKS | `team_players:CSK:2026` | `state:Maharashtra` | `season_records` | `orange_cap` | [ ] |
| 12 | 4 Apr | Sat | DC vs MI | `team_players:DC:2026` | `team_legends_batting:CSK` | `ranji:Mumbai` | `purple_cap` | [ ] |
| 13 | 5 Apr | Sun | SRH vs LSG | `team_players:SRH:2026` | `legends:india` | `head_coaches` | `winning_captain` | [ ] |
| 14 | 6 Apr | Mon | KKR vs PBKS | `team_players:KKR:2026` | `legends:overseas` | `fielding_records` | `costliest_player` | [ ] |
| 15 | 7 Apr | Tue | RR vs MI | `team_players:RR:2026` | `team_legends_bowling:MI` | `country:Australia` | `player_of_tournament` | [ ] |
| 16 | 8 Apr | Wed | DC vs GT | `team_players:DC:2026` | `orange_cap` | `state:Maharashtra` | `played_both:CSK:MI` | [ ] |
| 17 | 9 Apr | Thu | KKR vs LSG | `team_players:KKR:2026` | `purple_cap` | `ranji:Mumbai` | `multi_team:5` | [ ] |
| 18 | 10 Apr | Fri | RR vs RCB | `team_players:RR:2026` | `costliest_player` | `head_coaches` | `multi_team:7` | [ ] |
| 19 | 11 Apr | Sat | PBKS vs SRH | `team_players:PBKS:2026` | `player_of_tournament` | `top_run_scorers` | `longest_serving` | [ ] |
| 20 | 12 Apr | Sun | LSG vs GT | `team_players:LSG:2026` | `country:South Africa` | `top_wicket_takers` | `played_both:CSK:MI` | [ ] |

## Phase 2 — Match Days (13 Apr - 21 May)

| Ed | Date | Day | Match | Yellow | Green | Blue | Purple | Status |
|---|---|---|---|---|---|---|---|---|
| 21 | 13 Apr | Mon | RCB vs CSK | `team_players:RCB:2026` | `state:Maharashtra` | `highest_batting_avg` | `multi_team:5` | [ ] |
| 22 | 14 Apr | Tue | MI vs RR | `team_players:MI:2026` | `team_legends_batting:RCB` | `ranji:Mumbai` | `multi_team:7` | [ ] |
| 23 | 15 Apr | Wed | KKR vs DC | `team_players:KKR:2026` | `legends:india` | `head_coaches` | `most_ducks` | [ ] |
| 24 | 16 Apr | Thu | GT vs SRH | `team_players:GT:2026` | `legends:overseas` | `top_run_scorers` | `costliest_player` | [ ] |
| 25 | 17 Apr | Fri | CSK vs LSG | `team_players:CSK:2026` | `team_legends_bowling:RCB` | `country:South Africa` | `played_both:KKR:MI` | [ ] |
| 26 | 18 Apr | Sat | RCB vs MI | `team_players:RCB:2026` | `head_coaches` | `state:Maharashtra` | `multi_team:5` | [ ] |
| 27 | 19 Apr | Sun | PBKS vs KKR | `team_players:PBKS:2026` | `team_legends_batting:KKR` | `ranji:Mumbai` | `multi_team:7` | [ ] |
| 28 | 20 Apr | Mon | RR vs GT | `team_players:RR:2026` | `team_legends_bowling:KKR` | `country:England` | `longest_serving` | [ ] |
| 29 | 21 Apr | Tue | DC vs SRH | `team_players:DC:2026` | `head_coaches` | `state:Maharashtra` | `most_ducks` | [ ] |
| 30 | 22 Apr | Wed | CSK vs RCB | `team_players:CSK:2026` | `team_legends_batting:RR` | `ranji:Mumbai` | `played_both:RCB:MI` | [ ] |
| 31 | 23 Apr | Thu | MI vs PBKS | `team_players:MI:2026` | `team_legends_bowling:RR` | `country:New Zealand` | `multi_team:5` | [ ] |
| 32 | 24 Apr | Fri | LSG vs RR | `team_players:LSG:2026` | `head_coaches` | `state:Delhi` | `multi_team:7` | [ ] |
| 33 | 25 Apr | Sat | KKR vs GT | `team_players:KKR:2025` | `team_legends_batting:SRH` | `ranji:Mumbai` | `longest_serving` | [ ] |
| 34 | 26 Apr | Sun | SRH vs DC | `team_players:SRH:2026` | `team_legends_bowling:DC` | `country:England` | `played_both:CSK:MI` | [ ] |
| 35 | 27 Apr | Mon | RCB vs LSG | `team_players:RCB:2026` | `head_coaches` | `state:Delhi` | `multi_team:5` | [ ] |
| 36 | 28 Apr | Tue | CSK vs MI | `team_players:CSK:2026` | `team_legends_batting:DC` | `ranji:Delhi` | `multi_team:7` | [ ] |
| 37 | 29 Apr | Wed | GT vs PBKS | `team_players:GT:2026` | `team_legends_bowling:SRH` | `country:New Zealand` | `longest_serving` | [ ] |
| 38 | 30 Apr | Thu | RR vs KKR | `team_players:RR:2026` | `head_coaches` | `state:Delhi` | `most_ducks` | [ ] |
| 39 | 1 May | Fri | DC vs RCB | `team_players:DC:2026` | `team_legends_batting:PBKS` | `ranji:Delhi` | `played_both:CSK:MI` | [ ] |
| 40 | 2 May | Sat | MI vs SRH | `team_players:MI:2026` | `team_legends_bowling:PBKS` | `country:England` | `multi_team:5` | [ ] |
| 41 | 3 May | Sun | LSG vs CSK | `team_players:LSG:2026` | `head_coaches` | `state:Delhi` | `multi_team:7` | [ ] |
| 42 | 4 May | Mon | PBKS vs RR | `team_players:PBKS:2026` | `team_legends_batting:GT` | `ranji:Delhi` | `longest_serving` | [ ] |
| 43 | 5 May | Tue | GT vs KKR | `team_players:GT:2026` | `team_legends_bowling:LSG` | `country:New Zealand` | `played_both:CSK:MI` | [ ] |
| 44 | 6 May | Wed | SRH vs RCB | `team_players:SRH:2026` | `batting_coaches` | `state:Delhi` | `multi_team:5` | [ ] |
| 45 | 7 May | Thu | DC vs LSG | `team_players:DC:2025` | `team_legends_batting:GT` | `ranji:Delhi` | `multi_team:7` | [ ] |
| 46 | 8 May | Fri | MI vs CSK | `team_players:MI:2025` | `team_legends_bowling:GT` | `country:Sri Lanka` | `longest_serving` | [ ] |
| 47 | 9 May | Sat | RR vs SRH | `team_players:RR:2025` | `batting_coaches` | `state:Karnataka` | `most_ducks` | [ ] |
| 48 | 10 May | Sun | KKR vs PBKS | `team_players:KKR:2025` | `team_legends_batting:GT` | `ranji:Delhi` | `played_both:CSK:MI` | [ ] |
| 49 | 11 May | Mon | GT vs RCB | `team_players:GT:2026` | `team_legends_bowling:LSG` | `country:West Indies` | `multi_team:5` | [ ] |
| 50 | 12 May | Tue | DC vs CSK | `team_players:DC:2025` | `batting_coaches` | `state:Karnataka` | `multi_team:7` | [ ] |
| 51 | 13 May | Wed | MI vs GT | `team_players:MI:2025` | `team_legends_batting:GT` | `ranji:Delhi` | `longest_serving` | [ ] |
| 52 | 14 May | Thu | LSG vs KKR | `team_players:LSG:2025` | `country:Sri Lanka` | `top_run_scorers` | `played_both:CSK:MI` | [ ] |
| 53 | 15 May | Fri | SRH vs PBKS | `team_players:SRH:2026` | `state:Karnataka` | `top_wicket_takers` | `multi_team:5` | [ ] |
| 54 | 16 May | Sat | RCB vs RR | `team_players:RCB:2026` | `team_legends_batting:GT` | `ranji:Delhi` | `multi_team:7` | [ ] |
| 55 | 17 May | Sun | CSK vs DC | `team_players:CSK:2026` | `team_legends_bowling:LSG` | `batting_coaches` | `longest_serving` | [ ] |
| 56 | 18 May | Mon | KKR vs MI | `team_players:KKR:2025` | `country:West Indies` | `top_run_scorers` | `played_both:CSK:MI` | [ ] |
| 57 | 19 May | Tue | GT vs LSG | `team_players:GT:2026` | `state:Karnataka` | `top_wicket_takers` | `multi_team:5` | [ ] |
| 58 | 20 May | Wed | PBKS vs RR | `team_players:PBKS:2025` | `team_legends_batting:GT` | `ranji:Delhi` | `multi_team:7` | [ ] |
| 59 | 21 May | Thu | SRH vs DC | `team_players:SRH:2025` | `team_legends_bowling:LSG` | `batting_coaches` | `longest_serving` | [ ] |

## Playoffs (22-31 May)

| Ed | Date | Day | Match | Yellow | Green | Blue | Purple | Status |
|---|---|---|---|---|---|---|---|---|
| 60 | 22 May | Fri | — | `team_players:CSK:2008` | `country:Sri Lanka` | `top_run_scorers` | `played_both:RCB:MI` | [ ] |
| 61 | 23 May | Sat | — | `team_players:DC:2008` | `state:Karnataka` | `top_wicket_takers` | `multi_team:5` | [ ] |
| 62 | 24 May | Sun | — | `team_players:KKR:2008` | `team_legends_batting:GT` | `ranji:Karnataka` | `multi_team:7` | [ ] |
| 63 | 26 May | Tue | — | `team_players:MI:2008` | `team_legends_bowling:LSG` | `batting_coaches` | `longest_serving` | [ ] |
| 64 | 28 May | Thu | — | `team_players:PBKS:2008` | `country:West Indies` | `top_run_scorers` | `played_both:CSK:MI` | [ ] |
| 65 | 30 May | Sat | — | `team_players:RCB:2008` | `state:Tamil Nadu` | `top_wicket_takers` | `multi_team:5` | [ ] |
| 66 | 31 May | Sun | — | `team_players:RR:2008` | `team_legends_batting:GT` | `ranji:Karnataka` | `multi_team:7` | [ ] |

---

## Pool Usage Summary

| Spec | Uses | Cap | Remaining |
|------|------|-----|-----------|
| `multi_team:5` | 14 | ∞ | ∞ |
| `multi_team:7` | 14 | ∞ | ∞ |
| `head_coaches` | 11 | 11 | 0 |
| `longest_serving` | 11 | ∞ | ∞ |
| `played_both:CSK:MI` | 10 | ∞ | ∞ |
| `ranji:Mumbai` | 8 | 8 | 0 |
| `ranji:Delhi` | 8 | 8 | 0 |
| `team_legends_batting:GT` | 8 | ∞ | ∞ |
| `top_run_scorers` | 7 | 7 | 0 |
| `state:Maharashtra` | 7 | 7 | 0 |
| `team_players:CSK:2026` | 6 | 6 | 0 |
| `top_wicket_takers` | 6 | 7 | 1 |
| `batting_coaches` | 6 | 10 | 4 |
| `team_players:MI:2026` | 5 | 5 | 0 |
| `team_players:RCB:2026` | 5 | 5 | 0 |
| `team_players:KKR:2026` | 5 | 5 | 0 |
| `most_ducks` | 5 | 5 | 0 |
| `team_players:SRH:2026` | 5 | 5 | 0 |
| `team_players:RR:2026` | 5 | 5 | 0 |
| `team_players:GT:2026` | 5 | 5 | 0 |
| `state:Delhi` | 5 | 5 | 0 |
| `team_legends_bowling:LSG` | 5 | ∞ | ∞ |
| `state:Karnataka` | 5 | 5 | 0 |
| `costliest_player` | 4 | 4 | 0 |
| `team_players:PBKS:2026` | 4 | 4 | 0 |
| `team_players:LSG:2026` | 4 | 4 | 0 |
| `team_players:DC:2026` | 4 | 4 | 0 |
| `country:Australia` | 3 | 3 | 0 |
| `legends:india` | 3 | 3 | 0 |
| `legends:overseas` | 3 | 3 | 0 |
| `orange_cap` | 3 | 3 | 0 |
| `purple_cap` | 3 | 3 | 0 |
| `player_of_tournament` | 3 | 3 | 0 |
| `country:South Africa` | 3 | 3 | 0 |
| `country:England` | 3 | 3 | 0 |
| `country:New Zealand` | 3 | 3 | 0 |
| `team_players:KKR:2025` | 3 | 5 | 2 |
| `country:Sri Lanka` | 3 | 3 | 0 |
| `country:West Indies` | 3 | 3 | 0 |
| `winning_captain` | 2 | 2 | 0 |
| `played_both:RCB:MI` | 2 | ∞ | ∞ |
| `team_players:DC:2025` | 2 | 5 | 3 |
| `team_players:MI:2025` | 2 | 6 | 4 |
| `ranji:Karnataka` | 2 | 6 | 4 |
| `played_both:CSK:RCB` | 1 | ∞ | ∞ |
| `team_legends_batting:MI` | 1 | 1 | 0 |
| `batting_records` | 1 | 1 | 0 |
| `team_legends_bowling:CSK` | 1 | 1 | 0 |
| `bowling_records` | 1 | 1 | 0 |
| `season_records` | 1 | 1 | 0 |
| `team_legends_batting:CSK` | 1 | 1 | 0 |
| `fielding_records` | 1 | 1 | 0 |
| `team_legends_bowling:MI` | 1 | 1 | 0 |
| `highest_batting_avg` | 1 | 8 | 7 |
| `team_legends_batting:RCB` | 1 | 1 | 0 |
| `team_legends_bowling:RCB` | 1 | 1 | 0 |
| `played_both:KKR:MI` | 1 | ∞ | ∞ |
| `team_legends_batting:KKR` | 1 | 1 | 0 |
| `team_legends_bowling:KKR` | 1 | 1 | 0 |
| `team_legends_batting:RR` | 1 | 1 | 0 |
| `team_legends_bowling:RR` | 1 | 1 | 0 |
| `team_legends_batting:SRH` | 1 | 1 | 0 |
| `team_legends_bowling:DC` | 1 | 1 | 0 |
| `team_legends_batting:DC` | 1 | 1 | 0 |
| `team_legends_bowling:SRH` | 1 | 1 | 0 |
| `team_legends_batting:PBKS` | 1 | 1 | 0 |
| `team_legends_bowling:PBKS` | 1 | 1 | 0 |
| `team_legends_bowling:GT` | 1 | 1 | 0 |
| `team_players:RR:2025` | 1 | 5 | 4 |
| `team_players:LSG:2025` | 1 | 4 | 3 |
| `team_players:PBKS:2025` | 1 | 5 | 4 |
| `team_players:SRH:2025` | 1 | 4 | 3 |
| `team_players:CSK:2008` | 1 | 4 | 3 |
| `team_players:DC:2008` | 1 | 4 | 3 |
| `team_players:KKR:2008` | 1 | 5 | 4 |
| `team_players:MI:2008` | 1 | 5 | 4 |
| `team_players:PBKS:2008` | 1 | 5 | 4 |
| `team_players:RCB:2008` | 1 | 5 | 4 |
| `state:Tamil Nadu` | 1 | 4 | 3 |
| `team_players:RR:2008` | 1 | 4 | 3 |
